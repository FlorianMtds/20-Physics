...
var cameraProjectionMatrix = Fn(({ camera }) => {
  ...
  if (camera.isArrayCamera && camera.cameras.length > 0) {
    ...
    cameraProjectionMatrix3 = cameraProjectionMatrices.element(cameraIndex).toVar("cameraProjectionMatrix");
  } else {
...
var cameraViewMatrix = Fn(({ camera }) => {
  ...
  if (camera.isArrayCamera && camera.cameras.length > 0) {
    ...
    cameraViewMatrix3 = cameraViewMatrices.element(cameraIndex).toVar("cameraViewMatrix");
  } else {
...
var objectRadius = nodeProxy(Object3DNode, Object3DNode.RADIUS);
...
var StructTypeNode = class extends Node {
  ...
  getLength() {
    ...
    for (const member of this.membersLayout) {
      length3 += getLengthFromType(member.type);
    }
    return length3;
  }
  getMemberType(builder, name) {
...
var vertexColor = (index) => nodeObject(new VertexColorNode(index));
...
var ShadowBaseNode = class extends Node {
  ...
  setupShadowPosition({ context: context3, material }) {
    shadowPositionWorld.assign(material.shadowPositionNode || context3.shadowPositionWorld || positionWorld);
  }
  /**
...
var VSMPassVertical = Fn(({ samples, radius, size, shadowPass }) => {
  ...
  Loop({ start: int(0), end: int(samples), type: "int", condition: "<" }, ({ i }) => {
    ...
    const depth3 = shadowPass.sample(add(screenCoordinate.xy, vec2(0, uvOffset).mul(radius)).div(size)).x;
  ...
  updateShadow(frame) {
    ...
    renderer.setRenderObjectFunction((object, scene2, _camera2, geometry, material, group, ...params) => {
      if (object.castShadow === true || object.receiveShadow && shadowType === VSMShadowMap) {
        ...
        object.onBeforeShadow(renderer, object, camera, shadow3.camera, geometry, scene2.overrideMaterial, group);
        renderer.renderObject(object, scene2, _camera2, geometry, material, group, ...params);
        object.onAfterShadow(renderer, object, camera, shadow3.camera, geometry, scene2.overrideMaterial, group);
      }
    });
